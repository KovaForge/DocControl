...
- Documents: `GET /projects/{id}/documents`, `GET /projects/{id}/documents/{docId}`, `POST /projects/{id}/documents`, `POST /projects/{id}/documents/import` (lines: `CODE filename`), `DELETE /projects/{id}/documents/{docId}` (owner-only delete and counter reset), `DELETE /projects/{id}/documents` (owner-only purge)
- Audit: `GET /projects/{id}/audit`
- Settings: `GET/POST /projects/{id}/settings`, `GET/POST /projects/{id}/agent-tokens`, `DELETE /projects/{id}/agent-tokens/{tokenId}`
...
```
Features: project switcher, code catalog table, document generator/importer, members & roles, audit, settings, agent token management, AI recommend/interpret, management (document cleanup/purge), MFA setup with QR.
...
### Agent Tokens
Users can create revocable personal tokens on the Settings page. Tokens are shown once, stored server-side as hashes, and can be used by OpenClaw, Hermes, or direct API calls:
...
API requests may send the token with `Authorization: Bearer <token>`, `X-DocControl-Token`, or `x-api-key`. The token acts as the creating user and still respects that user's project roles.
